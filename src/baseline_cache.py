...
def _compute_baseline_from_df(
    ...
    if time_col not in df.columns:
        raise ValueError(f"La columna de tiempo '{time_col}' no existe en el dataframe.")
    ...
    counts = per_minute["alarm_count"] if not per_minute.empty else pd.Series(dtype=float)
...
def _load_df_from_sql(
    ...
    if flood_config.time_col in df.columns:
        df[flood_config.time_col] = pd.to_datetime(df[flood_config.time_col], errors="coerce")
    ...
    *,
    ...
    if df_alarms is None:
        if conn is None or db_config is None:
            raise ValueError("Debes proporcionar df_alarms o bien conn + db_config.")
